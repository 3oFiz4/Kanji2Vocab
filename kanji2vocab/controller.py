...
class AppController:
    ...
    def _fix_ai_response(self, response: str) -> str:
        ...
        if re.findall(useless_label, response, flags=re.MULTILINE) or re.search(r"Vocabulary\s+.+?", response):
            response = re.sub(useless_label, r"\1", response, flags=re.DOTALL)
            response = re.sub(r"^(?:Vocabulary\s*)+(.+?)$", r"\1", response, flags=re.MULTILINE)
            self.logger.log("[#af0][MODEL CORRECTION]: Removing useless label.[/]", "_")

        # Remove outlier template lines.
        if re.findall(r"===(.+)", response, flags=re.MULTILINE) or re.findall(r"{(.+)", response, flags=re.MULTILINE):
    ...
    async def _request_ai_explanations(self, vocab_list: list[str]) -> str:
        ...
        with Live(console=self.logger.console, screen=False) as live:
            while True:
                # Request AI response.
                api_response = await self.ai_client.request_chat(joined, prompt=prompt_text)

                # Handle quota errors by rotating key.
                if "402" in str(api_response):
                    self.logger.log("[#f00][API ERROR] (0 Kuota). Rotating to next key...[/]", "_")
                    self.ai_client.rotate_key()
                    continue

                # Log fatal API errors.
                if str(api_response).startswith("?!"):
                    self.logger.log(f"[#f00][FATAL API ERROR]:\n{api_response}\n---[/]", "_")
    ...
    def _color_vocab_by_verb_type(self, vocab: str, meaning: str) -> str:
        ...
        if re.search(r"v5", last_par, re.IGNORECASE):
            return vocab[:-1] + f'<span style="color:cyan">{vocab[-1]}</span>' if vocab else vocab
        if re.search(r"v1", last_par, re.IGNORECASE):
            return vocab[:-1] + f'<span style="color:red">{vocab[-1]}</span>' if vocab else vocab
        return vocab
    ...
    async def run_for_kanji(self, kanji: str, total_pages: int = 20, method: str = "c") -> None:
        ...
        if kanji_info and (kanji_info.onyomi or kanji_info.kunyomi or kanji_info.meaning or kanji_info.info):
            self.logger.log(
...
""",
        ...
        for idx, explanation in zip(sorted_indices, explanation_list):
            ...
            colored_vocab = self._color_vocab_by_verb_type(item.vocab, formatted_meaning)
    ...
    async def dispatch(self, args: CLIArgs) -> None:
        ...
        if args.action == "run" and args.kanji:
            ...
            await self.run_for_kanji(args.kanji, total_pages=args.total_pages or 20, method=method)
            return
